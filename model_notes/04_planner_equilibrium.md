...
CES demand, and frontier-gap learning. The benchmark comparison is designed to
evaluate the dual role of decentralized local industrial policy across industry
development stages: early industry-building versus scale-competition
misallocation.
...
optimizers, subject to the information and implementation constraints of a
...
The constrained planner internalizes cross-region spillovers, consumer or
final-good benefits from lower NEV prices, producer price/revenue compression,
duplicated capacity, fiscal costs, debt costs, and aggregate MPK losses. It may
not fully replicate decentralized local information, investment-attraction
capacity, multi-region experimentation, or private-capital mobilization. These
implementation advantages are part of the empirical trade-off.
...
- consumer or final-good benefits from lower NEV prices and producer
  price/revenue compression under CES demand;
...
$W^D<W^{CP}$ globally. It is to state sufficient conditions under which the
scale-competition or misallocation-dominant regime satisfies:
...
in the scale-competition region where installed capacity expands faster than
...
condition for the scale-competition stage; it implies that utilization falls
when capacity expands.

The function $Q(H^N)$ is a market-absorption mapping from installed capacity to
realized NEV sales. It is not a standalone inverse demand curve and does not
...
where $\mathcal B^A$ is the continuation value of NEV learning generated by
current realized NEV output. This term is the static counterpart of the dynamic
frontier-gap learning law. The term $\mathcal B_m^I$ captures
implementation-specific discovery and mobilization benefits. It can be larger
under decentralized local implementation if local information,
investment-attraction capacity, multi-region experimentation, or private-capital
mobilization are difficult for the centralized benchmark to reproduce. If these
advantages are assumed away, set $\mathcal B_D^I=\mathcal B_{CP}^I=0$ and the
model collapses to the full-information constrained-planner comparison.
...
2. both the consumer or final-good benefit from lower NEV prices and the national
   producer price/revenue compression created by excess NEV output;
3. the outside-sector capital opportunity cost of excess $H^N$;
4. fiscal-financing and debt-shadow costs that are not fully priced by a single
   local government;
5. cross-region learning spillovers;
...
- $\Delta_r^G\geq0$ is the marginal gap created by local output and employment
  weights that exceed their constrained-welfare marginal value in the
  overcapacity state;
...
$$
  ...
  $P_Q^N<0$. Thus $\Delta_r^P\geq0$ is a scale-competition condition, not a
  global assumption. If the consumer-benefit term dominates, $\Delta_r^P<0$ and
  the misallocation-dominant result requires stronger fiscal, capital, or
  political wedges;
...
- $\Delta_r^A\geq0$ is the marginal cross-region learning benefit not fully
  internalized by the local government, generated by
  $\partial\mathcal B^A/\partial Y_{-r}^N$ and the effect of $s_r$ on aggregate
  NEV learning;
...
learning, lower consumer prices, discovery, and implementation advantages can
make decentralized policy socially valuable, but duplicated-capacity,
consumer-benefit-adjusted price/revenue, fiscal, and MPK costs can dominate in
the scale-competition state.
...
### 6.7 Sufficient conditions for the misallocation-dominant regime

Assume the relevant policy range has:

1. **Positive firm response:**
   $h_s(s)>0$.
2. **Falling utilization in the scale-competition region:**
...
6. **Positive net local-planner wedge:**
   $\Delta(s)>0$, meaning that political-output, consumer-benefit-adjusted
   price/revenue, outside-capital, and fiscal wedges dominate learning and
...
model for the misallocation-dominant regime.

This is a sufficient condition for the misallocation-dominant
regime. It is not imposed as a global conclusion. If
...
competition can dominate the centralized benchmark through learning, discovery,
consumer gains, and implementation advantages.
...
In the scale-competition region, assume:
...
misallocation-dominant regime, fiscal, fixed-cost, low-utilization,
price/revenue, and capital-misallocation costs dominate consumer, learning,
discovery, and implementation benefits. Under that additional net-cost
condition:
...
The static model therefore delivers the aggregate-misallocation pattern only under the
misallocation-dominant sufficient conditions:
...
investment-attraction capacity, multi-region experimentation, consumer benefits,
...
5. **Proposition 3:** Given Proposition 2 and the net-cost-dominance condition,
   ...
   consumer, learning, discovery, and implementation benefits are credited.
